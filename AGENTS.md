...
- 首版已合并进 main（PR #1），2026-09-11 用户人工验收通过。2026-09-12 批准的三项扩展（预存 SSH 连接、多命名绑定、可选目录边界）在 `feat/named-bindings` 分支，自动测试全绿（含真实 VM Python 套件），尚未人工验收。最终内网离线现场验收仍未完成；无 root 与旧 glibc 约束不变。
...
- `docs/design/large-file-spec.md`：大文件读写、远端搜索、续传、状态清理与二次配置的扩展规格，尚未实施。
