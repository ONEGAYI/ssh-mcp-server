...
每个绑定的工作区 MCP 都提供 `remote_help`：零参数返回该工作区的完整使用指引（工作流顺序、后台任务、恢复与确认规则）。它是纯本地静态文本，不连 SSH，连接异常时同样可调用；Agent 拿不准工作流规则时应先调它，server instructions 中也有引导。
...
分发一律使用上述离线包（含 Node 运行时与全部依赖，目标机无需联网与 npm install）；不提供轻量分发包（2026-09-14 确立的分发纪律，package-dist 已移除）。
