...
export async function runWorkspaceServer(profile: string): Promise<void> {
  ...
  for (const [name, action, description] of [
    ...
    ["remote_search", "file_search", "Literal UTF-8 content search, maximum 64 MiB scanned and 64 KiB returned. Skips .git and binary/oversized files; does not implement .gitignore or regex. Does not authorize writes."],
  ]) register(name, description, { sessionId, path: path.optional(), pattern: z.string().optional(), filePattern: z.string().optional(), limit: count, cursor: z.string().optional() }, input => files.call(action, input.sessionId, input));
  register("remote_edit", "Apply unique exact replacements inside known ranges; preserve BOM, permissions and CRLF. On success use the returned new readToken for further edits without rereading: known ranges shift with your edits, unread gaps remain protected. External changes still invalidate tokens. If rereadRequired=true the edit was committed but token renewal failed; read before continuing and do not repeat the edit blindly.",
  ...
  register("remote_write", "Create only if absent (create=true), or overwrite after complete current-version read. Supply exactly one of text or base64 data.",
    { sessionId, path, readToken, create: z.boolean().optional(), text: z.string().optional(), data: z.string().optional() }, input => files.call("file_write", input.sessionId, input));
  register("remote_upload", "Upload local file up to 16 MiB. Existing remote target requires complete read; create=true refuses overwrite.",
    { sessionId, path, localPath: z.string(), create: z.boolean().optional(), readToken }, input => files.upload(input.sessionId, input as any));
