...
2026-09-13 票据 [#9](https://github.com/ONEGAYI/ssh-mcp-server/issues/9) 已按 [ADR 0006](../adr/0006-large-file-version-checks.md) 实施读取侧：版本改为 Linux 元数据观察（取消全文摘要），读取取消 16 MiB 体积上限并改为有界流式交付，新增 metadataOnly 与字节游标续读，readToken 引入 3 天闲置过期判定（到期记录的回收执行已随 #17 实施）。票据 [#10](https://github.com/ONEGAYI/ssh-mcp-server/issues/10) 已实施写侧：精确替换与整体覆盖取消 16 MiB 全文边界，改为分块匹配与流式拼接提交，整体覆盖按 [ADR 0008](../adr/0008-explicit-version-bound-overwrite.md) 走显式 `overwrite` + `expectedVersion`。删除与移动工具及其 16 MiB 全文快照路径已随 #20 移除（ADR 0007）。下文「读取与版本」「写入提交」描述当前实现。
...
2026-09-12 已接受、2026-09-13 随 [票据 #20](https://github.com/ONEGAYI/ssh-mcp-server/issues/20) 实施的工具调整见 [ADR 0007](../adr/0007-file-management-through-shell.md)：remote_move、remote_delete、remote_mkdir、remote_rmdir 已从 MCP 工具列表与远端 helper 分发中移除，移动、删除、创建目录及删除空目录改用远端 Shell（登记执行任务）完成。Shell 移动删除不再拥有 readToken 保护；这不代表任何未来删除请求自动获授权——实际操作仍遵循当前任务范围和用户指令。其余文件工具的读取凭据、已读区间、版本覆盖与写保护规则不变。helper 镜像按内容摘要隔离部署，各版本客户端使用自己的镜像，远端不为退役动作保留兼容分发（旧镜像由 #17 维护轮按升级纪律回收）。
...
移动、删除、创建目录与删除空目录不再是文件工具（ADR 0007 / #20）：改用远端 Shell 命令（登记执行任务）完成，该路径不设 readToken、已读范围与版本检查，防误操作依靠 Agent 操作规范及所在客户端的命令审查机制。递归删除、覆盖整个目录树同样只经 Shell，不设工具级保护。首版对写入经过符号链接或涉及多硬链接的目标返回明确不支持的行为仅覆盖存留的文件工具；后续可独立扩展。
...
**未实测边界**：上述行为已在 WSL（假 rg 枚举后端 + 真实 Python 遍历）自动化验证；CentOS 7 VM 无 rg，仅实测 `python-walk` 路径。真实 ripgrep 二进制的 `--files` 输出顺序与旗标行为未经自动化覆盖（实现按路径排序后消费，不依赖其输出顺序）；内网离线现场已随 v2.0.0 于 2026-09-14 通过用户验收。
...
**未实测边界**：上述行为已在 WSL（真 GNU grep + 假 rg/假 grep 后端）与 CentOS 7 VM（GNU grep 降级路径）自动化验证；内网离线现场已随 v2.0.0 于 2026-09-14 通过用户验收。
...
- 分段读取可在同一版本下累计范围。局部编辑须覆盖其匹配文本（#10 起匹配按字节区间判定，跨流式分块边界同样生效）；整文件覆盖已改为显式版本绑定（见「写入提交」）；删除与移动不再是文件工具，其原“完整已知范围”要求随工具一并退役（#20 / ADR 0007，改用远端 Shell）。外部修改后原凭据失效，不继承旧范围；本服务确认的精确编辑按下条续期。
...
- 该续期规则仅适用于精确编辑。`write/upload` 不因成功自动签发新凭据，也不把整文件隐藏读取当成调用方已读；`download` 走 `grantRead=false` 传输路径，不签发凭据也不受 56 KiB 文本预算约束（每块仍受调用方 `maxBytes` 约束）。删除与移动入口已随 #20 退役，Shell 侧同等操作不进入凭据体系。
...
- 临时文件登记量按流式输出的精确字节数（原大小减去被替换区间加新字节）计算，额度检查随登记在账本锁内完成；删除/移动的全文快照路径已随 #20 移除，16 MiB 内存快照链路不复存在，`MAX_FILE_BYTES` 仅保留为 inline 写入请求预算。
...
新内容先写入目标所在文件系统的临时文件，再进入远端协作锁：核对目标身份与预期版本，保留支持的权限位，提交变更。创建采用不会覆盖已出现目标的操作，不使用先判断后无条件覆盖的降级实现。

本服务的并发写入互相协调；任何阶段冲突必须保留原目标，返回 `FILE_CONFLICT`。临时文件清理只能作用于本次操作明确拥有的文件。跨文件系统移动曾由专用工具拒绝并解释；该工具退役后，跨文件系统操作由 Shell 命令（如 `mv` 自身的复制降级或拒绝行为）承担，本服务不作工具级约束。
...
- 版本握手与排空门槛：升级时旧入口 `start` 在协议激活前仍可创建任务（升级窗口），激活后仅保留对已存在任务的幂等观察重放，创建一律拒绝 `PROTOCOL_UPGRADE_REQUIRED`。存在**观察上仍在运行**的旧协议任务时，握手与登记返回 `LEGACY_TASKS_PENDING` 并列出阻塞标识，不静默强杀；旧任务保持可查询、可取消、可确认。判活与 status 同源（describe）：worker 已消失而记录仅形态上未终态的旧任务观察为 unknown，不再写入任何数据，也不阻塞切换——其记录按「生命周期与回收」节的 unknown 期限在协议激活后回收（#20 收紧；此前仅按 state.json 字段判定，死记录会永久阻塞升级）。激活检查与旧入口创建经同一固定协议锁串行，两种协议不会同时接收新写入；文件锁槽的旧 per-path 锁切换门槛（`LOCK_SWITCH_BLOCKED`，#8）继续独立生效。
...
- **读取凭据回收（#17）**：`<状态根>/reads/` 下过期（`now > expiresAt`，与读取路径 `READ_TOKEN_EXPIRED` 判定同时钟、同语义——晚到的成功操作不复活已过期记录）的 `<32hex>.json` 凭据记录被物理删除（时间戳字段缺失或类型非法属不可判定，保守跳过保留，损坏绝不触发删除），指向已删或已过期凭据的 `index-*.json` 会话路径索引同轮回收（名字序保证凭据先于索引处理，悬空索引不留到下轮；grant_read 对悬空索引本就按首读处理）。回收后旧凭据编辑按 `READ_REQUIRED`（记录不可用）拒绝；重读所需片段签发只覆盖新窗口的新凭据，旧已读范围不复活。
...
- **回收后拒绝**：记录删除后旧标识不再落入任何创建分支——`task_start` 报 `REQUEST_EXPIRED_OR_UNKNOWN`，传输各动作经 `_load_record` 同码拒绝；登记后执行协议（#7）保证这一点，#16 的验收即覆盖"回收后重放被拒"。legacy（v1）任务记录仅在 v2 协议激活后删除：未激活工作区的 v1 记录只清日志不删记录，避免升级窗口内旧请求经 legacy start 重跑；#20 起该门槛覆盖全部删除分支（终态与 unknown 形态一致），且激活排空判活与 status 同源——死 worker 的 v1 记录进 unknown 阶段（迁移观察时间起算 30 天）后不再阻塞切换，激活后按原时间回收，删除后 legacy start 重放按 `PROTOCOL_UPGRADE_REQUIRED` 拒绝。
...
第 3–4 项已用真实 ZCode 运行时、正式任务服务、正式恢复钩子与 SSH 持久任务完成受控模型集成验证。用户于 2026-09-11 另报告真实 ZCode 桌面人工验收通过：钩子注入、重启存活、原任务 wait 挂接、无重复执行、结果确认均符合预期，详见 progress.md。内网离线部署已随 v2.0.0 于 2026-09-14 通过用户验收。
