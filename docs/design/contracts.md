...
- 分段读取可在同一版本下累计范围。局部编辑须覆盖其匹配文本；整文件覆盖、删除、移动须已完整读取。文件中途变化则原凭据失效，不跨版本累计。
...
- 一次成功变更后旧凭据失效。新版本可返回，但后续修改仍需新的有效读取；不能将成功写入自动当成重新读取。
...
第 3–4 项已用真实 ZCode 运行时、正式任务服务、正式恢复钩子与 SSH 持久任务完成集成验证；模型响应由本机固定响应服务控制。验证了原任务不重跑、通知对应恢复后的等待、处理确认后不再列为待处理。实际桌面项目首次信任和自然模型行为仍需人工验收。
